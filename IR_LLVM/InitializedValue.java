...
public class InitializedValue {

    private final int dim;

    private final LLVMType elementType;

    private final ArrayList<ArrayList<Integer>> values; // dim == 0 时, lens.get(0).get(0) is the value, dim == 1时, len.get(0) is the value

    public InitializedValue(int dim, ArrayList<ArrayList<Integer>> values) {
        this.dim = dim;
        this.values = values;
    ...
    public void addInitial(InitializedValue initializedValue) {
        if (getDim() == 1) {
            values.get(0).add(initializedValue.getValues().get(0).get(0));
        } else {
            values.add(initializedValue.getValues().get(0));
        }
    }
