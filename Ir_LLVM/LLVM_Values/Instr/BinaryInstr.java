...
public class BinaryInstr extends Instr {
    ...
    public BinaryInstr(LLVMType type, LLVM_Value oprand1, LLVM_Value oprand2, op opcode) {
        super(type, Tasks.isSetNameAfterGen ? "" : LLVM_Builder.getInstance().genVirtualRegNum()); //TODO:nameGen
        this.opcode = opcode;
        this.addValue(oprand1);
        this.addValue(oprand2);
    }

    public BinaryInstr(LLVMType type, LLVM_Value operand, op opcode) { //for -x or +x
    ...
    public void genMIPS() {
        ...
        MipsBuilder.getInstance().addAsm(asm0);
        //结果先统一存在K0里面
        ...
        if (ALU.contains(opcode)) {
            ...
            MipsBuilder.getInstance().addAsm(mulDivAsm);
            if (opcode == BinaryInstr.op.SREM) {
                HLAsm mfhi = new HLAsm(HLAsm.Op.mfhi, tar);
                MipsBuilder.getInstance().addAsm(mfhi);
            } else {
    ...
    private boolean optimizeDiv(LLVM_Value v, int val) {
        ...
        MipsBuilder.getInstance().addAsm(mul);
        ...
        MipsBuilder.getInstance().addAsm(srl);
        if (!this.useReg) {
            MemITAsm sw = new MemITAsm(MemITAsm.Op.sw, tar, Register.SP, this.offset);
            MipsBuilder.getInstance().addAsm(sw);
        }
